...
This walkthrough covers some of the same concepts from Jamie Montgomery's [guide](https://github.com/jamiecmontgomery/spatial-analysis-R) on spatial data in R, which is how I first learned to use the `sf` package. This walkthrough will cover some of the same concepts and also some different ones.
...
I could potentially take the above dataframe elsewhere, write it to a csv, etc.
...
Wildfires also generate smoke. One source of spatial wildfire smoke data is the National Oceanic and Atmospheric Administration (NOAA) Hazard Mapping System (HMS) smoke product. Drawn from satellite images, this data gives daily shapefiles for wildfire smoke extent.
